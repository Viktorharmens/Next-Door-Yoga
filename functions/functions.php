<?php
	...
	// =================================================
	// Custom searchform element with empty submit item
	// =================================================
	function custom_searchform( $form ) {
		$search_query = get_search_query();
		$form = '<form role="search" method="get" id="searchform" class="searchform" action="/" >
					<a href="#" class="searchform__close js-toggle-search">&times;</a>
					<input type="text" placeholder="' . __('Zoeken...', 'visualmasters') . '" name="s" id="searchinput" class="searchform__input" />
					<input type="submit" id="searchsubmit" class="searchform__submit" value="' . __('Zoeken', 'visualmasters') . '" />
				 </form>';
		return $form;
	}
	...
	function the_breadcrumbs() {
		if ( function_exists('yoast_breadcrumb') && get_field( 'show_breadcrumbs', get_the_id() ) ) { 
			yoast_breadcrumb();
		}
	}
	...
	function get_pagination() {
		...
		return '<div class="pagination">' . 
				str_replace('prev page-numbers', 'prev', 
	...
	function get_page_part( $part, $file, $content = null ) {
		// Load the file 
	    if( locate_template( '/parts/' . $part . '/' . $file . '.php', false, false ) != '' ) {
	...
	// ===========================================
	// Print out the current language and toggler
	// ===========================================
	function the_current_language( ) {   
	    ...
	    echo '<a href="#" class="language__current js-toggle-language">
	    		  <img src="' . $curr_lang['country_flag_url'] . '" />
	    		  <em>' . $curr_lang['translated_name'] . '</em>
	    		  <i class="language__toggle"></i>
	    	  </a>'; $curr_lang;
	}

	// ===================================================
	// Get the SVG icon and set it as DOM HTML
	// ===================================================
	function get_svg_icon( $slug ) {
		if( file_exists( get_stylesheet_directory() . '/dist/img/social/' . $slug . '.svg' ) ) {
			
			$iconfile = new DOMDocument();
			$iconfile->load( get_stylesheet_directory_uri() . '/dist/img/social/' . $slug . '.svg' );
